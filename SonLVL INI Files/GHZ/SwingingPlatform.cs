...
namespace S1ObjectDefinitions.GHZ
{
	class SwingingPlatform : ObjectDefinition
	{
		...
		{
			imgwreckingball = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../Graphics - Compressed/GHZ Giant Ball.nem", CompressionType.Nemesis), "mappings/GHZ Ball.asm", 1, 2);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../Graphics - Compressed/GHZ Swinging Platform.nem", CompressionType.Nemesis), "mappings/Swinging Platforms (GHZ).asm", labels[i], i == 1 ? 0 : 2));
		}
